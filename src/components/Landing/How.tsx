    ...
    <section className="py-16 sm:py-24 bg-gray-50" id="how-it-works">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative">
          ...
          <div className="hidden lg:block absolute inset-y-0 left-1/2 w-0.5 bg-blue-200 transform -translate-x-1/2"></div>

          {workflowSteps.map((step, index) => (
            <div 
              key={step.step} 
              className={`relative flex flex-col lg:flex-row items-center py-10 ${index % 2 === 0 ? 'lg:flex-row-reverse' : ''}`}
            >
              
              {/* Content Column */}
              <div className="lg:w-5/12">
                {/* <div className={`p-6 bg-white rounded-xl shadow-xl border border-gray-100 ${index % 2 === 0 ? 'lg:text-right' : 'lg:text-left'}`}> */}
                  ...
                  <ul className="space-y-3 text-gray-700">
                    {step.details.map((detail, i) => (
                      <li key={i} className="flex items-start">
                        {/* Bullet Icon (Hidden on Right-aligned text) */}
                        <svg className={`flex-shrink-0 w-5 h-5 text-blue-500 mt-1 ${index % 2 === 0 ? 'lg:hidden' : 'lg:block'}`} fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                          <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd"></path>
                        </svg>
                        {/* Bullet Icon (Hidden on Left-aligned text) */}
                        <svg className={`flex-shrink-0 w-5 h-5 text-blue-500 mt-1 ${index % 2 === 0 ? 'lg:block' : 'lg:hidden'}`} fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                          <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd"></path>
                        </svg>
              ...
              {/* Empty Column (or Image Placeholder if needed) */}
              <div className="lg:w-5/12">
                {/* Placeholder for future content or to maintain spacing */}
              </div>
            </div>
          ))}
        </div>
